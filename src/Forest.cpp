...
void Forest::initR(std::unique_ptr<Data> input_data, uint mtry, uint num_trees, std::ostream* verbose_out, uint seed,
    ...
    bool regularization_usedepth, bool treewise_importance) {
  ...
  init(std::move(input_data), mtry, "", num_trees, seed, num_threads, importance_mode, min_node_size,
      ...
      regularization_factor, regularization_usedepth, treewise_importance);
...
void Forest::init(std::unique_ptr<Data> input_data, uint mtry, std::string output_prefix,
    ...
    uint max_depth, const std::vector<double>& regularization_factor, bool regularization_usedepth, bool treewise_importance) {
  ...
  this->treewise_importance = treewise_importance;
...
void Forest::grow() {
  ...
  if (treewise_importance) {
    variable_importance_treewise.resize(num_trees);
    for (size_t i = 0; i < num_trees; ++i) {
      variable_importance_treewise[i].resize(num_independent_variables, 0);
    }
  } else {
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    if (treewise_importance) {
      trees[i]->grow(&variable_importance_treewise[i]);
    } else {
...
void Forest::computePermutationImportance() {
  ...
  for (size_t i = 0; i < num_trees; ++i) {
    if (treewise_importance) {
      trees[i]->computePermutationImportance(variable_importance_treewise[i], variance, variable_importance_casewise);
    } else {
...
void Forest::computeTreePermutationImportanceInThread(uint thread_idx, std::vector<double>& importance,
  ...
  if (thread_ranges.size() > thread_idx + 1) {
    for (size_t i = thread_ranges[thread_idx]; i < thread_ranges[thread_idx + 1]; ++i) {
      if (treewise_importance) {
        trees[i]->computePermutationImportance(variable_importance_treewise[i], variance, importance_casewise);
      } else {
