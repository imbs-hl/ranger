...
void TreeClassification::findBestSplitValueSmallQ(size_t nodeID, size_t varID, size_t num_classes,
  ...
  for (size_t i = 0; i < possible_split_values.size(); ++i) {
    ...
    if (i < (possible_split_values.size() - 1) && (min_bucket->size() == 1 && (n_left < (*min_bucket)[0] || n_right < (*min_bucket)[0]))) {
      continue;
    }
...
void TreeClassification::findBestSplitValueLargeQ(size_t nodeID, size_t varID, size_t num_classes,
  ...
  for (size_t i = 0; i < num_unique; ++i) {
    ...
    // Stop if minimal bucket size reached
    if (i < (num_unique - 1) && (min_bucket->size() == 1 && (n_left < (*min_bucket)[0] || n_right < (*min_bucket)[0]))) {
      continue;
    }
