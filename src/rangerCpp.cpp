...
Rcpp::List rangerCpp(uint treetype, Rcpp::NumericMatrix& input_x, Rcpp::NumericMatrix& input_y,
    ...
    bool node_stats, std::vector<double>& time_interest, bool use_time_interest, bool any_na, std::string plink_file) {
  ...
  try {
    ...
    forest->initR(std::move(data), mtry, num_trees, verbose_out, seed, num_threads,
        ...
        order_snps, max_depth, regularization_factor, regularization_usedepth, node_stats, plink_file);
