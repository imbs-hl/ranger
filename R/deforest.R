...
deforest.ranger <- function(object, which.trees = NULL, warn = TRUE, ...) {
  ...
  # Throw error in the case of survial trees (at least for now)
  if (object$treetype == "Survival") {
    stop("The `deforest()` function does not currently work for survival ",
         "forests.", call. = FALSE)
  }
  ...
  # Update `predictions` and related components
  object$predictions[] <- NA  # replace each entry with NA
  object$prediction.error <- NA
  if (!is.null(object$r.squared)) {
    object$r.squared <- NA
  }
