...
library(rangerts)
library(survival)
context("rangerts_surv")

## Initialize the random forest for survival analysis
rg.surv <- rangerts(Surv(time, status) ~ ., data = veteran, num.trees = 10)

## Basic tests (for all random forests equal)
test_that("survival result is of class rangerts with 15 elements", {
  expect_is(rg.surv, "rangerts")
...
test_that("Alternative interface works for survival", {
  rf <- rangerts(dependent.variable.name = "time", status.variable.name = "status", data = veteran, num.trees = 10)
...
test_that("Matrix interface works for survival", {
  rf <- rangerts(dependent.variable.name = "time", status.variable.name = "status", data = data.matrix(veteran), write.forest = TRUE, num.trees = 10)
  expect_equal(rf$treetype, "Survival")
...
test_that("Matrix interface prediction works for survival", {
  ...
  rf <- rangerts(dependent.variable.name = "time", status.variable.name = "status", data = dat, write.forest = TRUE, num.trees = 10)
  expect_silent(predict(rf, dat))
})

test_that("growing works for single observations, survival", {
  rf <- rangerts(Surv(time, status) ~ ., veteran[1, ], write.forest = TRUE, num.trees = 10)
  expect_is(rf$survival, "matrix")
})

test_that("predict works for single observations, survival", {
  rf <- rangerts(Surv(time, status) ~ ., veteran, write.forest = TRUE, num.trees = 10)
...
test_that("Survival error without covariates", {
  expect_error(rangerts(Surv(time, status) ~ ., veteran[, c("time", "status")], num.trees = 5),
...
test_that("Survival error for competing risk data", {
  ...
  expect_error(rangerts(y = sobj, x = veteran[, 1:2], num.trees = 5),
