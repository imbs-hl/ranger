...
library(rangerts)
context("rangerts_inbag")

## Tests
test_that("Inbag count matrix if of right size, with replacement", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE)
...
test_that("Inbag count matrix if of right size, without replacement", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, replace = FALSE, keep.inbag = TRUE)
  expect_equal(dim(data.frame(rf$inbag.counts)),
              c(nrow(iris), rf$num.trees))
})

test_that("Inbag count matrix if of right size, with replacement, weighted", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, case.weights = runif(nrow(iris)), keep.inbag = TRUE)
  expect_equal(dim(data.frame(rf$inbag.counts)),
              c(nrow(iris), rf$num.trees))
})

test_that("Inbag count matrix if of right size, without replacement, weighted", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, replace = FALSE, case.weights = runif(nrow(iris)), keep.inbag = TRUE)
  expect_equal(dim(data.frame(rf$inbag.counts)),
              c(nrow(iris), rf$num.trees))
})
...
test_that("Number of samples is right sample fraction, replace=FALSE, default", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = FALSE)
...
test_that("Number of samples is right sample fraction, replace=FALSE, 0.3", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = FALSE, sample.fraction = 0.3)
  num.inbag <- sapply(rf$inbag.counts, function(x) {
    sum(x > 0)
  })
  sample.fraction <- mean(num.inbag/nrow(iris))
...
test_that("Number of samples is right sample fraction, replace=TRUE, default", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = TRUE)
  num.inbag <- sapply(rf$inbag.counts, function(x) {
    sum(x > 0)
  })
...
test_that("Number of samples is right sample fraction, replace=TRUE, 0.5", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = TRUE, sample.fraction = 0.5)
  num.inbag <- sapply(rf$inbag.counts, function(x) {
    sum(x > 0)
  })
...
test_that("Number of samples is right sample fraction, replace=FALSE, 0.3, weighted", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = FALSE, sample.fraction = 0.3, case.weights = runif(nrow(iris)))
  num.inbag <- sapply(rf$inbag.counts, function(x) {
    sum(x > 0)
  })
  sample.fraction <- mean(num.inbag/nrow(iris))
...
test_that("Number of samples is right sample fraction, replace=TRUE, 0.5, weighted", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, keep.inbag = TRUE, replace = TRUE, sample.fraction = 0.5, case.weights = runif(nrow(iris)))
  num.inbag <- sapply(rf$inbag.counts, function(x) {
    sum(x > 0)
  })
...
test_that("Manual inbag selection selects correct observations", {
  ...
  rf <- rangerts(Species ~ ., iris, num.trees = 5, replace = FALSE, keep.inbag = TRUE, inbag = inbag)
  expect_equal(rf$inbag.counts,
               inbag)

  inbag <- replicate(5, round(runif(nrow(iris), 0, 5)), simplify = FALSE)
  rf <- rangerts(Species ~ ., iris, num.trees = 5, replace = TRUE, keep.inbag = TRUE, inbag = inbag)
  expect_equal(rf$inbag.counts,
               inbag)
})
