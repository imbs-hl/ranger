library(ranger)
context("ranger for time series")
...
test_that("Fonctional when time series parameters are active", {
  ...
  expect_silent(ranger(Sepal.Length ~ ., data = dat,
                       block.size = 10))
  ...
  rf_iid <- ranger(Sepal.Length ~ ., data = dat)
  rf_no  <- ranger(Sepal.Length ~ ., data = dat,
                   bootstrap.ts = "nonoverlapping", 
                   block.size = 10)
  ...
  rf_temp <- ranger(Sepal.Length ~ ., data = dat,
                   bootstrap.ts = "nonoverlapping", 
                   block.size = 10, by.end = F)
  rf_inv  <- ranger(Sepal.Length ~ ., data = dat,
                   bootstrap.ts = "nonoverlapping", 
                   block.size = 10)
