...
rg.prob <- rangerts(Species ~ ., data = iris.train, write.forest = TRUE, probability = TRUE)
...
test_that("growing works for single observations, probability prediction", {
  expect_warning(rf <- rangerts(Species ~ ., iris[1, ], write.forest = TRUE, probability = TRUE),
...
test_that("save.memory option works for probability", {
  rf <- rangerts(Species ~ ., data = iris, probability = TRUE, save.memory = TRUE)
  expect_equal(rf$treetype, "Probability estimation")
})

test_that("predict works for single observations, probability prediction", {
  rf <- rangerts(Species ~ ., iris, write.forest = TRUE, probability = TRUE)
  ...
  rf <- rangerts(Species ~ ., dat, write.forest = TRUE, probability = TRUE)
...
test_that("Probability estimation works correctly if labels are reversed", {
  ...
  ## Train
  rf <- rangerts(dependent.variable.name = "label", data = dat, probability = TRUE,
               write.forest = TRUE, num.trees = 5)
  rf.rev <- rangerts(dependent.variable.name = "label", data = dat.rev, probability = TRUE,
                   write.forest = TRUE, num.trees = 5)
...
test_that("Probability estimation works correctly if first or second factor level empty", {
  expect_warning(rf <- rangerts(Species ~ ., iris[51:150, ], probability = TRUE),
  ...
  expect_warning(rf <- rangerts(Species ~ ., iris[c(101:150, 51:100), ], probability = TRUE),
                 "^Dropped unused factor level\\(s\\) in dependent variable\\: setosa\\.")
...
test_that("No error if unused factor levels in outcome", {
  expect_warning(rf <- rangerts(Species ~ ., iris[1:100, ], num.trees = 5, probability = TRUE),
...
test_that("predict.all for probability returns 3d array of size samples x classes x trees", {
  rf <- rangerts(Species ~ ., iris, num.trees = 5, write.forest = TRUE, probability = TRUE)
