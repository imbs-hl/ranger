...
test_that("Variable excluded if - in formula", {
  rf <- rangerts(Species ~ . -Petal.Length, iris, num.trees = 5)
  expect_equal(sort(rf$forest$independent.variable.names),
               sort(c("Sepal.Length", "Sepal.Width", "Petal.Width")))
})

test_that("Interaction included if : in formula", {
  rf <- rangerts(Species ~ Petal.Length + Sepal.Length:Sepal.Width, iris, num.trees = 5)
  expect_equal(sort(rf$forest$independent.variable.names),
...
test_that("Variable excluded if - in formula", {
  rf <- rangerts(Surv(time, status) ~ . - celltype - age, veteran, num.trees = 5)
  expect_equal(sort(rf$forest$independent.variable.names),
               sort(c("trt", "karno", "diagtime", "prior")))
})

test_that("Interaction included if : in formula", {
  rf <- rangerts(Surv(time, status) ~ celltype + age:prior, veteran, num.trees = 5)
  expect_equal(sort(rf$forest$independent.variable.names),
...
test_that("Error if interaction of factor variable included", {
  expect_error(rangerts(Surv(time, status) ~ celltype*prior, veteran, num.trees = 5),
...
test_that("Working if dependent variable has attributes other than names", {
  ...
  expect_silent(rangerts(data = iris2, dependent.variable = "Sepal.Width"))
})
...
test_that("Same result with x/y interface, classification", {
  set.seed(300)
  rf_formula <- rangerts(Species ~ ., iris, num.trees = 5)

  set.seed(300)
  rf_xy <- rangerts(y = iris[, 5], x = iris[, -5], num.trees = 5)
...
test_that("Same result with x/y interface, regression", {
  set.seed(300)
  rf_formula <- rangerts(Sepal.Length ~ ., iris, num.trees = 5)

  set.seed(300)
  rf_xy <- rangerts(y = iris[, 1], x = iris[, -1], num.trees = 5)
...
test_that("Same result with x/y interface, survival", {
  set.seed(300)
  rf_formula <- rangerts(Surv(time, status) ~ ., veteran, num.trees = 5)

  set.seed(300)
  rf_xy <- rangerts(y = veteran[, c(3, 4)], x = veteran[, c(-3, -4)], num.trees = 5)
...
test_that("Column order does not change prediction", {
  ...
  rf <- rangerts(dependent.variable.name = "Species", data = iris)
...
#   set.seed(1000)
#   rf1 <- rangerts(Species ~ ., tb, num.trees = 5)
#
#   set.seed(1000)
...
#   skip_if_not_installed("tibble")
#   set.seed(1000)
#   rf1 <- rangerts(dependent.variable.name = "Species", data = tb, num.trees = 5)
#
#   set.seed(1000)
#   rf2 <- rangerts(dependent.variable.name = "Species", data = iris, num.trees = 5)
#
#   expect_equal(rf1$prediction.error, rf2$prediction.error)
#
#   pred1 <- levels(iris$Species)[rf1$predictions[!is.na(rf1$predictions)]]
