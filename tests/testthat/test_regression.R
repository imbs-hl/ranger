...
library(rangerts)
context("rangerts_reg")

## Initialize the random forest for regression
rg.reg <- rangerts(Sepal.Length ~ ., data = iris)

## Basic tests (for all random forests equal)
test_that("regression result is of class rangerts with 14 elements", {
  expect_is(rg.reg, "rangerts")
...
test_that("Alternative interface works for regression", {
  rf <- rangerts(dependent.variable.name = "Sepal.Length", data = iris)
  expect_equal(rf$treetype, "Regression")
})

test_that("Matrix interface works for regression", {
  rf <- rangerts(dependent.variable.name = "Sepal.Length", data = data.matrix(iris), write.forest = TRUE)
  expect_equal(rf$treetype, "Regression")
...
test_that("Matrix interface prediction works for regression", {
  ...
  rf <- rangerts(dependent.variable.name = "Sepal.Length", data = dat, write.forest = TRUE)
  expect_silent(predict(rf, dat))
})

test_that("save.memory option works for regression", {
  rf <- rangerts(Sepal.Length ~ ., data = iris, save.memory = TRUE)
  expect_equal(rf$treetype, "Regression")
})

test_that("predict.all for regression returns numeric matrix of size n x trees", {
  rf <- rangerts(Petal.Width ~ ., iris, num.trees = 5, write.forest = TRUE)
...
test_that("Alternative interface regression prediction works if only independent variable given, one independent variable", {
  ...
  rf <- rangerts(dependent.variable.name = "y", data = dt, num.trees = 5, write.forest = TRUE)
  expect_silent(predict(rf, dt))
  expect_silent(predict(rf, dt[, 1, drop = FALSE]))

  dt2 <- data.frame(y = rbinom(n, 1, 0.5), x = runif(n))
  rf <- rangerts(dependent.variable.name = "y", data = dt2, num.trees = 5, write.forest = TRUE)
...
test_that("Variance splitting not working on classification data", {
  expect_error(rangerts(Species ~ ., iris, splitrule = "variance"))
})

## Splitrule
test_that("default splitrule is variance for regression", {
  set.seed(42)
  rf1 <- rangerts(Sepal.Length ~ ., iris, num.trees = 5)

  set.seed(42)
  rf2 <- rangerts(Sepal.Length ~ ., iris, num.trees = 5, splitrule = "variance")
...
test_that("splitrule extratrees is different from variance for regression", {
  set.seed(42)
  rf1 <- rangerts(Sepal.Length ~ ., iris, num.trees = 5, splitrule = "extratrees")
...
test_that("splitrule maxstat is different from variance for regression", {
  set.seed(42)
  rf1 <- rangerts(Sepal.Length ~ ., iris, num.trees = 5, splitrule = "maxstat")
